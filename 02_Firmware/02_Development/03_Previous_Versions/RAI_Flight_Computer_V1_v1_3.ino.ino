...
// - Confirmed apogee detection
// - APOGEE + DESCENT states
...
// ============================================================
// FLIGHT LOGIC SETTINGS
// ============================================================

// Bench-development launch threshold
const float LAUNCH_G_THRESHOLD = 1.40;
const int LAUNCH_CONFIRM_SAMPLES = 3;

// Apogee confirmation
const float APOGEE_DROP_THRESHOLD = 0.60;   // meters below peak
const int APOGEE_CONFIRM_SAMPLES = 5;

// Avoid apogee logic immediately after launch
const unsigned long MIN_ASCENT_TIME = 1500; // ms
...
void initializeHardware()
{
  ...
  deploymentServo.attach(SERVO_PIN, 500, 2400);
...
void readSensors()
{
  ...
  absoluteAltitude = bmp.readAltitude(1013.25);

  relativeAltitude = absoluteAltitude - baseAltitude;
  ...
  gyroX = (gxRaw - gyroOffsetX) / 131.0;
  gyroY = (gyRaw - gyroOffsetY) / 131.0;
  gyroZ = (gzRaw - gyroOffsetZ) / 131.0;
...
void detectLaunch()
{
  ...
  if (launchCounter >= LAUNCH_CONFIRM_SAMPLES)
  {
    flightState = ASCENT;
...
void detectApogee()
{
  ...
  // Don't evaluate apogee immediately after launch
  if (timeSinceLaunch < MIN_ASCENT_TIME)
  {
    return;
  }
  ...
  if (dropFromPeak >= APOGEE_DROP_THRESHOLD)
  {
    apogeeCounter++;

    Serial.print("Apogee confirm: ");
    Serial.print(apogeeCounter);
    Serial.print("/");
    Serial.println(APOGEE_CONFIRM_SAMPLES);
  }
  else
  {
    apogeeCounter = 0;
  ...
  if (apogeeCounter >= APOGEE_CONFIRM_SAMPLES)
  {
    flightState = APOGEE;
    ...
    // Apogee notification only.
    // Servo does NOT deploy automatically in V1.3.
...
void updateApogeeState()
{
  ...
  {
    // Hold APOGEE state briefly so it can be seen/logged
...
void startServoTest()
{
  ...
  deploymentServo.write(SERVO_DEPLOYED);
...
void updateServo()
{
  if (
    servoActive &&
    millis() - servoStartTime >= SERVO_TEST_TIME
  )
  {
    deploymentServo.write(SERVO_STOWED);
...
void updateBuzzer()
{
  if (
    buzzerActive &&
    millis() - buzzerStartTime >= BUZZER_TEST_TIME
  )
  {
    digitalWrite(BUZZER_PIN, LOW);
...
void processSerialCommands()
{
  ...
  if (command == "ARM" && flightState == IDLE)
  {
    flightState = ARMED;
  ...
  if (command == "RESET")
  {
    ...
    Serial.println("FLIGHT LOGIC RESET");
...
void updateDisplay()
{
  ...
  display.print("G:");
  display.println(accelerationMagnitude, 2);
...
void printSensorData()
{
  ...
  {
    Serial.print("T+: ");
    Serial.print(timeSinceLaunch / 1000.0, 2);
    Serial.println(" s");
  }
}
